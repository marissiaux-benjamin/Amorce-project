@props(['label_text','type','placeholder','name','for','id','field'])
<div class=" relative flex flex-col mb-6">
    <label class="mb-2 text-lg font-bold" for="{{ $for }}">
        {{ $label_text }}
    </label>
    <input wire:model="{{ $field }}" name="{{ $name }}" id="{{ $id }}" type="{{ $type }}"
           placeholder="{{ $placeholder }}" class="border-2 py-3 pl-11 rounded-xl text-[#2E2E2E] bg-transparent">
