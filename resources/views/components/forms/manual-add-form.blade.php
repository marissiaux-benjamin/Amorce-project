<div x-data="{ open: @entangle('isOpened') }"
     ...
     class="fixed inset-0 z-50 flex justify-end overflow-y-auto hover:border-gray-300 backdrop-blur-sm filter">
    <section class="flex flex-col justify-center w-1/2 h-full bg-[#E9E9E9] px-16 relative">
        ...
        <x-buttons.cross-button route="{{ route('comptes.index') }}"/>
        <x-forms.manual-add-form action="{{ route('transaction.store') }}" method="post"/>
